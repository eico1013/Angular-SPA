...
import {Chart as ChartType, ChartConfig, ChartSettings} from 'chart.js';

import {isBlank, log} from './index';

declare var Chart: {
    new (context: CanvasRenderingContext2D, config: ChartConfig): ChartType;
...
export default class DiagrammService {
     ...
     */
    @log
    createChart(chartElement: HTMLCanvasElement, config: ChartConfig): ChartType
        |undefined {
    ...
    getBackgroundColor(idx: number): string {
        const colorHighlight: ColorHighlight =
     ...
     */
    getHoverBackgroundColor(idx: number): string {
        const backgroundColor: ColorHighlight =
            this.backgroundColors.get(idx % 3) as ColorHighlight;
