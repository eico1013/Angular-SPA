...
export class KundenService {
     ...
     * @param kunde Das Kunden-Objekt, das gepuffert wird.
     * @return void
     */
    ...
    find(suchkriterien: KundeForm): void {
        ...
        const nextFn: (response: Response) => void = (response) => {
            ...
            let kunden: Array<Kunde> = this.responseToArrayBuch(response);
        ...
        const errorFn: (err: Response) => void = (err) => {
            ...
            if (status === 400) {
                ...
                if (isBlank(body)) {
                    ...
                    // z.B. [PARAMETER][findByName.name][Bei einem ...][x]
    ...
    findById(id: string): void {
        // Gibt es ein gepufferte Kunden mit der gesuchten ID?
        if (isPresent(this._kunde) && this._kunde._id === id) {
            console.log('KundenService.findById(): Kunde gepuffert');
     ...
     * @param neuenKunden Das JSON-Objekt mit dem neuen Kunden
     * @param successFn Die Callback-Function fuer den Erfolgsfall
     * @param errorFn Die Callback-Function fuer den Fehlerfall
     */
    @log
    save(
        neuenKunden: Kunde, successFn: (location: string|undefined) => void,
        errorFn: (status: number, text: string) => void): void {
        neuenKunden.datum = moment(new Date());

        const uri: string = this.baseUriKunden;
        const body: string = JSON.stringify(neuenKunden.toJSON());
    ...
    createBarChart(chartElement: HTMLCanvasElement): void {
        ...
        const nextFn: ((response: Response) => void) = (response) => {
            ...
            const kunden: Array<Kunde> = this.responseToArrayKunde(response);
            const labels: Array<string> =
                kunden.map((kunde: Kunde) => kunde._id) as Array<string>;
            console.log('KundenService.createBarChart(): labels: ', labels);
            const ratingData: Array<number> =
                kunden.map((kunde: Kunde) => kunde.rating) as Array<number>;
    ...
    private suchkriterienToSearchParams(suchkriterien: KundeForm):
        ...
        if (!isEmpty(suchkriterien.name)) {
            searchParams.set('name', suchkriterien.name as string);
        }
